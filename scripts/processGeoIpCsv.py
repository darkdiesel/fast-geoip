...
def getCSVFileName(file):
    return os.environ["MAXMIND_EDITION_ID"].replace("-CSV", "") + file
...
def generateLocationsFile():
    ...
    location_items= []
    max_item_length=0
    with open(os.path.join(RAW_DATABASE_DIR, getCSVFileName("-Locations-en.csv")), encoding='utf-8') as locations_file:
...
def generateBlockFiles(geoname_ids):
    ...
    ips = b'['
    ipIndex = []
    with open(os.path.join(RAW_DATABASE_DIR, getCSVFileName("-Blocks-IPv4.csv")), encoding='utf-8') as blocks_file:
